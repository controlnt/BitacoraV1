...
/*if (isset($_POST['btn_crear'])) {
    ...
    $query = "SELECT codigo FROM empresas WHERE codigo BETWEEN '10000' AND '99999' ORDER BY codigo DESC LIMIT 1";
    $stmt = $bd->prepare($query);

    if (!$stmt) {

        echo "Error en la preparacion del query: (" . $bd->errno . ")" . $bd->error;
        ...
        if (!$exito) {
            ...
            $exito = $stmt->bind_result($codigo);

            if (!$exito) {

                echo "Error en la vinculacion de resultados1: (" . $stmt->errno . ")" . $stmt->error;
                ...
                if (!$exito) {
                    ...
                    echo "Error en la vinculacion de resultados con la variables: (" . $stmt->errno . ")" . $stmt->error;
                    ...
                    $query = "SELECT codigo FROM empresas WHERE codigo = ? LIMIT 1";
                    $stmt = $bd->prepare($query);
                    ...
                    if (!$stmt) {
                        ...
                        $exito = $stmt->bind_param('i', $codigo_s);
                        ...
                        if (!$exito) {
                            ...
                            if (!$exito) {
                                ...
                                echo "Error en la ejecuion: (" . $stmt->errno . ")" . $stmt->error;

                            }else {

                                $exito = $stmt->bind_result($codigo_d);

                                if (!$exito) {

                                    echo "Error en la asociacion de resultados: (" . $stmt->errno . ")" . $stmt->error;
                                    ...
                                    if (!$exito) {
                                        ...
                                        echo "Error en la vinculacion de resultados2: (" . $stmt->errno . ")" . $stmt->error;
                                        $stmt->close();
                                        ...
                                        $query = "INSERT INTO empresas(empresa, nit, codigo) VALUES (?, ?, ?)";
                                        $stmt->prepare($query);
                                        ...
                                        if (!$exito) {
                                            ...
                                            $exito = $stmt->bind_param('sii', $nombre, $nit, $codigo);
                                        ...
                                        $query = "SELECT codigo FROM empresas WHERE codigo BETWEEN '10000' AND '99999' ORDER BY codigo ASC LIMIT 1";
                                        $stmt = $bd->prepare($query);
                                        ...
                                        if (!$stmt) {
                                            ...
                                            if (!$exito) {
                                                ...
                                                $exito = $stmt->bind_result($codigo_2);

                                                if (!$exito) {

                                                    echo "Error en la vinculacion de resultados3: (" . $stmt->errno . ")" . $stmt->error;
                                                    ...
                                                    if (!$stmt) {
                                                        ...
                                                        $exito = $stmt->bind_param('sii', $nobmre, $nit, $codigo_r);
